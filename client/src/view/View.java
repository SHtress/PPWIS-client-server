...
public class View {
    ...
    private void newDoc() {
        ...
        ((Button) newDocWindow.getDialogPane().lookupButton(newDocWindow.getButtonTypes().get(0))).setOnAction(ae -> {
            ...
            try {
                MainWindow.clientOutputStream.writeObject("newDoc");
                MainWindow.clientOutputStream.writeObject(examNumber);
                MainWindow.clientOutputStream.writeObject(entitiesNumber);
                controller.setModel((ArrayList<Student>) MainWindow.clientInputStream.readObject(), examNumber);
    ...
    private void openDoc() {
        try {
            MainWindow.clientOutputStream.writeObject("openDoc");
            List<String> chooseList = (List<String>) MainWindow.clientInputStream.readObject();
            Stage primaryStage = new Stage();
            primaryStage.setTitle("Открыть документ");
            final Popup popup = new Popup();
            popup.setX(300);
            popup.setY(200);
            ListView<String> view = new ListView();
            view.setItems(FXCollections.observableArrayList(chooseList));

            Button buttonOpen = new Button("Открыть");
            buttonOpen.setOnAction(new EventHandler<javafx.event.ActionEvent>() {
                @Override
                public void handle(javafx.event.ActionEvent actionEvent) {
                    String res;
                    res = view.getSelectionModel().getSelectedItem();
                    if (res != null) {
                        try {
                            MainWindow.clientOutputStream.writeObject(res);
                            ArrayList<Student> newList = (ArrayList<Student>) MainWindow.clientInputStream.readObject();
                            tableElement.rewriteDefaultList(newList);
            ...
            VBox layout = new VBox(10, view, buttonOpen);
    ...
    private void saveDoc() {
        try {
            MainWindow.clientOutputStream.writeObject("saveDoc");
            Stage primaryStage = new Stage();
            primaryStage.setTitle("Сохранить документ");
            ...
            saveButton.setOnAction(new EventHandler<javafx.event.ActionEvent>() {
                ...
                public void handle(javafx.event.ActionEvent actionEvent) {
                    try {
                        MainWindow.clientOutputStream.writeObject(saveName.getText());
                        primaryStage.close();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
            ...
            VBox layout = new VBox(10, saveName, saveButton);
            primaryStage.setScene(new Scene(layout));
            primaryStage.show();
    ...
    private void addItems() {
        try {
            MainWindow.clientOutputStream.writeObject("addStud");
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        ((Button) addItemWindow.getDialogPane().lookupButton(addItemWindow.getButtonTypes().get(0))).setOnAction(ae -> {
            Student studentToAdd = new Student(new SNP(surnameField.getText(), nameField.getText(), patronymField.getText()), groupField.getText(), examList);
            try {
                MainWindow.clientOutputStream.writeObject(studentToAdd);
                int size =(Integer) MainWindow.clientInputStream.readObject();
                ArrayList<Student> newList = new ArrayList<>();
                for (int i = 0; i <size; i++) {
                    newList.add((Student) MainWindow.clientInputStream.readObject());
                }
                tableElement.rewriteDefaultList(newList);
                tableElement.resetToDefaultItems();
            } catch (Exception e) {
    ...
    private class RequestElement {
        ...
        public RequestElement(WindowType windowType) {
            ...
            searchButton.setOnAction(ae -> {
                ArrayList<Student> studentList = search();
        ...
        public ArrayList<Student> search() {
            ...
            try {
                MainWindow.clientOutputStream.writeObject(selectedItem);
                MainWindow.clientOutputStream.writeObject(criteriaList);
                int size =(Integer) MainWindow.clientInputStream.readObject();
                for (int i = 0; i <size; i++) {
                    result.add((Student) MainWindow.clientInputStream.readObject());
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private void searchItems() {
        try {
            MainWindow.clientOutputStream.writeObject("search");
    ...
    private void deleteItems() {
        try{MainWindow.clientOutputStream.writeObject("delete");}
        catch (Exception e){e.printStackTrace();};
        ...
        ((Button) deleteItemsWindow.getDialogPane().lookupButton(deleteItemsWindow.getButtonTypes().get(0))).setOnAction(ae -> {
            ...
            try {
                ArrayList<Student> listToDelete = requestElement.search();
                System.out.println(listToDelete);
                ArrayList<Student> listClient =(ArrayList<Student>)MainWindow.clientInputStream.readObject();

                MainWindow.clientOutputStream.writeObject(listToDelete.size());
                for (int i = 0; i <listToDelete.size(); i++) {
                   MainWindow.clientOutputStream.writeObject(listClient.indexOf(listToDelete.get(i)));
                }
                ArrayList<Student> newlist= new ArrayList<>();
                int size =(Integer) MainWindow.clientInputStream.readObject();
                for (int i = 0; i <size; i++) {
